<template>
  <div class="app">
    ...
    <main class="todo-container">
      ...
      <ul v-else class="todo-list">
        ...
        >
          ...
          <button class="btn-delete" @click="deleteTodo(todo.id)" title="Delete">
            &times;
          </button>
        </li>
      </ul>
      ...
      <div class="todo-footer">
        ...
        <button class="btn-text" @click="clearCompleted">
          Clear completed
        </button>
      </div>
    </main>
    ...
    <footer class="app-footer">
      ...
      <p class="powered">Powered by <strong>Forge</strong> + Vue + Python</p>
    </footer>
  </div>
</template>
...
async function addTodo() {
  ...
  try {
    await invoke('todo_add', { text: text.value.trim() })
    text.value = ''
    await loadTodos()
  } catch (err) {
...
.app-header h1 { font-size: 36px; font-weight: 700; letter-spacing: -1px; }
...
.todo-check input { opacity: 0; width: 0; height: 0; }
.checkmark {
  position: absolute;
  top: 0; left: 0;
